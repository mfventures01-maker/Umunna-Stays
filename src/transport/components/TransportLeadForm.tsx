...
const TransportLeadForm: React.FC<TransportLeadFormProps> = ({
  ...
  const [form, setForm] = useState({
    ...
    guest_phone: '',
    guest_email: '',
    city: selectedCity || vendor?.coverage_cities?.[0] || 'Asaba',
    property_id: '',
    pickup_location: '',
    dropoff_location: '',
    date_needed: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Analytics.lead('transport');
      Analytics.reservationSubmit('transport_booking');
      Analytics.whatsappClick('transport_form');
