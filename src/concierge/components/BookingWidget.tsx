...
const BookingWidget: React.FC<BookingWidgetProps> = ({ property, appData }) => {
  ...
  const bookingSummary = useMemo(() => {
    ...
    const booking = property.booking || { whatsapp_prefill: '', minimum_stay: 1 };
    const minStay = booking.minimum_stay || 1;
  ...
  const generateWhatsAppUrl = () => {
    const host = property.host || { host_name: '', host_whatsapp: '', host_photo_url: '' };
    const booking = property.booking || { whatsapp_prefill: '', minimum_stay: 1 };
    const base = `https://wa.me/${host.host_whatsapp || appData.meta.whatsapp_main_number}`;
    let message = booking.whatsapp_prefill;
          ...
          <div className="w-12 h-12 bg-gray-100 rounded-full flex-shrink-0 overflow-hidden">
            {(property.host ?? {}).host_photo_url ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-[#C46210] font-black text-xl">
                {(property.host?.host_name ?? "H").charAt(0)}
              </div>
            )}
          </div>
          <div>
            <p className="text-[10px] font-black uppercase text-gray-400">Hosted by</p>
            <p className="font-bold text-gray-900">{property.host?.host_name ?? "Umunna Host"}</p>
          </div>
        </div>
      </div>
    </div>
