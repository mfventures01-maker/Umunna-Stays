...
const Food: React.FC<FoodProps> = ({ appData }) => {
  const vendors = getFoodVendors(appData) || [];
  const mainVendor = vendors[0] || { name: 'Judith Amazing Kitchen', description: 'Gourmet Local Dining', menu_url: '#' };
  ...
  const categories = useMemo<string[]>(() => {
    const cats = Array.from(new Set((menuItems ?? []).map(item => item.category))) as string[];
    return ['All', ...cats];
  }, [menuItems]);
